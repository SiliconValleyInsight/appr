...
module.exports = function postDeploy() {
  ...
  }?release-channel=${utils.getExpChannelName()}`;
  const expUrlForQRCode = `https://exp.host/@${config.expUsername}/${
    utils.readPackageJSON().name
  }?release-channel=${utils.getExpChannelName()}`;
  ...
  if (config.githubPullRequestId) {
    ...
    request.post(
      ...
      (error, response) => {
        if (error) {
          console.error(
            "Failed to post comment to GitHub, an error occurred",
            error
          );
          ...
          console.log(
            `Posted message to GitHub PR #${config.githubPullRequestId}`
          );
        }
      }
    );
